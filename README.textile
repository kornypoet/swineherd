...
The above @script.run@ will pass Hadoop and Pig settings through @PIG_OPTS@ and run the following command line: 
...
<pre><code>./swineherd --map_tasks=20 --run_mode=local --script_param.in_path=fips_to_state.tsv foo.pig.erb</code></pre>

Script variables are specified using @--script_param@
